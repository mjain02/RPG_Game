...
TEST(RemoveTest, RemoveFromEnd)
{
	...
	insertItem(ht, 4, m[1]);
	insertItem(ht, 8, m[2]);
	insertItem(ht, 12, m[3]);
	void* data = removeItem(ht, 12);
	EXPECT_EQ(m[3], data);
	destroyHashTable(ht);
}
...
TEST(RemoveTest, RemoveFromHead)
{
	...
	void* data = removeItem(ht, 0);
	EXPECT_EQ(m[0], data);
	EXPECT_EQ(m[2], getItem(ht, 8));
	destroyHashTable(ht);
}

TEST(RemoveTest, RemoveFromCenter)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	insertItem(ht, 8, m[2]);
	insertItem(ht, 12, m[3]);
	EXPECT_EQ(m[2], removeItem(ht, 8));
	EXPECT_EQ(m[3], getItem(ht, 12));
	destroyHashTable(ht);
}

TEST(RemoveTest, RemoveFromListInvalidKey)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	insertItem(ht, 8, m[2]);
	insertItem(ht, 12, m[3]);
	EXPECT_EQ(NULL, removeItem(ht, 5));
	EXPECT_EQ(m[3], getItem(ht, 12));
	destroyHashTable(ht);
}
///////////////////
...
TEST(InsertTest, MakeList)
{
	...
	destroyHashTable(ht); //removes m items
}


TEST(ReplaceTest, ReplaceValue)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	insertItem(ht, 0, m[0]);
	insertItem(ht, 1, m[1]);
	void* oldValue = removeItem(ht, 1);
	insertItem(ht, 1, m[2]);
	//replace m[1] with m[2]
...
TEST(ReplaceTest, RemoveAndInsert)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
	...
	insertItem(ht, 0, m[0]);
	insertItem(ht, 4, m[1]); //form a linked list
	EXPECT_EQ(m[0], getItem(ht, 0));
	void* oldValue = removeItem(ht, 4);
	EXPECT_EQ(m[1], oldValue);
    insertItem(ht, 1, oldValue); //re-insert
	EXPECT_EQ(m[1], getItem(ht, 1));

	destroyHashTable(ht);
}

TEST(DeleteTest, DeleteItem)
{
	HashTable* ht = createHashTable(hash, BUCKET_NUM);
